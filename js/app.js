...
function renderPlayer(name) {
  ...
  for (const roundKey of ROUND_ORDER) {
    ...
    for (const match of round.matches) {
      ...
      let predPenNote = '';
      if (pred) {
        predScore = formatScore(pred.home, pred.away);
        if (pred.pen_winner) {
          predPenNote = `<div class="pen-note pred-pen">🥅 ${pred.pen_winner}</div>`;
        }
      }
      ...
      // Penalty indicator on actual result
      const penNote = played && result.penalties_winner
        ? `<div class="pen-note result-pen">🥅 ${result.penalties_winner}</div>` : '';

      const tr = document.createElement('tr');
      tr.className = rowClass;
      tr.innerHTML = `
        <td class="team-cell home">${teamDisplay(match.home)}</td>
        <td class="score-col pred-score ${scored.exactScore ? 'exact' : ''}">${predScore}${predPenNote}</td>
